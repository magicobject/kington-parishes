...
1. **[templates/header.html](templates/header.html)**, **[templates/safeguard-strip.html](templates/safeguard-strip.html)**, **[templates/footer.html](templates/footer.html)**, **[templates/page.html](templates/page.html)** — the shared page shell (nav, the "concerned about a child or adult's safety" banner, footer, `<head>`) with `{{PLACEHOLDER}}` tokens. Each is optional per page — see `header`, `safeguardStrip` and `footer` flags in `src/pages.config.mjs` (the 404 page skips the header and safeguard-strip but keeps the footer, so the mediawright credit and contact details still show there; the safeguarding page skips the safeguard-strip since it would just link to itself).
...
**[.githooks/pre-commit](.githooks/pre-commit)** — [/updates.html](src/pages/updates.html) shows the current build number, like `Current build: 2026.08.29.003` (format `yyyy.mm.dd.NNN`, where `NNN` counts commits made that day, stored in [build-number.json](build-number.json)). This is maintained automatically, not by hand. On every commit, the hook:

1. Runs [scripts/bump-build-number.mjs](scripts/bump-build-number.mjs), which increments today's counter in `build-number.json`.
2. Runs `npm run build`, regenerating `public/`. The build number itself only appears on `/updates.html` — it used to be in every page's footer and in every `?v=` cache-buster, which meant every commit rewrote every page. Cache-busting now uses a hash of each file's own content instead (`css/style.css?v=ab25f78f35`), added automatically to every same-origin `<link href>`/`<script src>` a page loads, so a page's HTML only changes when something it actually loads has changed. `sitemap.xml` likewise has no `<lastmod>` (it was only ever the build date, not a real per-page date).
...
The CSP is `default-src 'self'` plus the minimum extra it actually needs: `https://fonts.googleapis.com`/`https://fonts.gstatic.com` for the Google Fonts stylesheet, `'unsafe-inline'` on `style-src` only, because several pages use inline `style="..."` attributes (a full refactor to CSS classes felt like unnecessary scope for a proof-of-concept), and `frame-src https://www.google.com` for the homepage's Find Us map embed. `script-src` stays `'self'` with no exceptions — there are no inline `<script>` blocks anywhere in `src/pages/` or `templates/`, only external files (`main.js`, `calendar.js`, `calendar-events.js`), so it doesn't need one.

This file has no effect from `npm run serve` — the local static test server doesn't apply it, since `_headers` is a Cloudflare-only convention. **This bit us for real**: the Find Us map embed was added and shipped without re-checking the CSP, and silently rendered as Google's "This content is blocked" error in production — `default-src 'self'` has no `frame-src` fallback for framing `https://www.google.com`, and `npm run serve` never would have caught it. The fix, and the way to actually catch this class of bug locally, is `npx wrangler dev` — it serves `public/` through the real Cloudflare Workers asset handler, `_headers` included, unlike the plain static test server. Re-run it (and watch the browser console for CSP violations) after any change to `_headers` or to what a page loads or embeds.
...
- **[footer.spec.ts](test/footer.spec.ts)** — `/updates.html` shows a correctly-formatted build number, no page footer does, and every page with a footer shows the "Site by mediawright.uk" credit.
- **[asset-versions.spec.ts](test/asset-versions.spec.ts)** — every local stylesheet, script and icon a page loads carries a `?v=` matching that file's content hash, and the sitemap has no `<lastmod>`.
