...
- **The other four churches' portal pages have no People section yet** — `CHURCH_OFFICERS` only has a `kington` key. When those churches' officers are known, add a key for that slug and add the matching `{{PEOPLE:<slug>}}` section to that church's portal page (copy the pattern from `church-kington.html`).
