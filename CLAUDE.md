...
Each church has its own portal page (`church-kington.html`, `church-titley.html`, `church-old-radnor.html`, `church-kinnerton.html`, `church-huntington.html`), reachable from `our-churches.html` two ways: a "Visit the ... page →" text link in that church's `.links-row`, and the church's own photo (`.parish-photo`), both pointing at the same URL. Not in the primary nav — reachable only from `our-churches.html`, same pattern as Donate/Parish News Archive. The photo link carries an `aria-label` ("Visit the ... page") rather than relying on the `<img>`'s own `alt` (a scenic description, not a link purpose) as its accessible name — keep both in sync if you change either.
