...
// Today's date, used to filter the calendar's Event structured data down to
// what's still upcoming — Google penalizes Event rich results that list
// events that have already happened, and this list only gets regenerated on
// a rebuild, so "upcoming" means "as of this build" until the next one.
...
// Leaflet's own stylesheet + script, needed by every page with a map (the
// homepage and each church portal page) — one {{LEAFLET}} token rather than
// the same two tags hand-copied onto each. Each page still loads its own
// map script (churches-map.js or church-map.js) after it.
const leafletPartial = read('templates/leaflet.html').trim();

// Cache-busting for every same-origin stylesheet, script and icon a page
// loads: `<link href="/css/style.css">` becomes
// `<link href="/css/style.css?v=<hash>">`, where the hash is taken from that
// file's own content. A page's HTML therefore only changes when something it
// actually loads has changed — unlike the old ?v=<build number>, which
// rewrote every page on every commit whether or not the CSS/JS had moved,
// and left browsers re-downloading unchanged files after each deploy.
// Runs over the finished HTML, so any <link href="/..."> or <script
// src="/..."> a page adds picks this up without anyone having to remember a
// ?v= — a reference to a file that doesn't exist fails the build instead.
...
// "edit the source, not this file" rule as public/*.html. Written before the
// pages themselves, so calendar.html's ?v= hash (see versionAssetUrls) is
// taken from this build's copy, not the previous one. Pre-expanded here
